    ...
    public KeycloakTestUtils(Map<String, String> envs) {
        ...
        try {
            new EnvironmentVariables(envs).execute(() -> {
                kc.set(Keycloak.builder().setVersion("26.6.3").addDependency("idnord.keycloak", "keycloak-spi-limit-resend-email", spiVersionFromPomXml).start("start-dev", "--http-port=" + kcHttpPort));
            });
        } catch (Exception e) {
